...
const roleByEmail: Record<string, UserRole> = {
  ...
  "billing@example.test": "BILLING_STAFF",
};
...
export const authHandlers = [
  rest.post(
    ...
    async (request, response, context) => {
      ...
      if (!role || body.password !== "synthetic-password")
        return response(context.status(401));
      return response(
        context.status(200),
        context.json({ token: createSyntheticToken(role, body.email) }),
      );
    },
  ),
  ...
  rest.get(`${API_BASE_URL}/test/unauthorized`, (_request, response, context) =>
    response(context.status(401)),
  ),
  rest.get(`${API_BASE_URL}/test/forbidden`, (_request, response, context) =>
