...
(function () {
  ...
  const roleLabels = {
    ADMIN: 'Administrator', CLINICIAN: 'Clinician', NURSE: 'Nurse',
    REGISTRATION_STAFF: 'Registration staff', RECEPTIONIST: 'Receptionist',
    BILLING_STAFF: 'Billing staff', PHARMACIST: 'Pharmacist', LAB_STAFF: 'Laboratory staff',
    PRIVACY_OFFICER: 'Privacy officer', AUDITOR: 'Auditor', ANALYST: 'Analyst'
  };
  ...
  const staffNav = [
    ['Work queue', '#/staff/work-queue', 'WQ', ['ADMIN','CLINICIAN','NURSE','RECEPTIONIST','BILLING_STAFF','PHARMACIST','LAB_STAFF']],
    ['Patients', '#/staff/patients', 'PT', ['ADMIN','CLINICIAN','REGISTRATION_STAFF']],
    ['Schedule', '#/staff/schedule', 'SC', ['ADMIN','CLINICIAN','REGISTRATION_STAFF']],
    ['Clinical chart', '#/staff/patients/pat-1047/summary', 'CC', ['ADMIN','CLINICIAN','NURSE']],
    ['Billing', '#/staff/patients/pat-1047/billing', 'BL', ['ADMIN','BILLING_STAFF']],
    ...
    ['Notifications', '#/staff/notifications', 'NT', ['ADMIN','CLINICIAN','REGISTRATION_STAFF','BILLING_STAFF']],
  ...
  const allScreens = [
    ['Entry states', [
      ['Sign in', '#/login'], ['Access denied', '#/forbidden'], ['System states', '#/states']
    ]],
    ['Patient portal', patientNav.map(([label, href]) => [label, href])],
    ['Staff operations', [
      ['Work queue', '#/staff/work-queue'], ['Patient registry', '#/staff/patients'],
      ['Patient summary', '#/staff/patients/pat-1047/summary'], ['Demographics', '#/staff/patients/pat-1047/demographics'],
      ['Encounters', '#/staff/patients/pat-1047/encounters'], ['Medications', '#/staff/patients/pat-1047/medications'],
      ['Laboratory', '#/staff/patients/pat-1047/labs'], ['Patient billing', '#/staff/patients/pat-1047/billing'],
      ['Schedule', '#/staff/schedule'], ['Insurance', '#/staff/insurance'],
      ['Pharmacy', '#/staff/pharmacy'], ['Inventory', '#/staff/inventory'],
      ['Notifications', '#/staff/notifications'], ['Compliance', '#/staff/compliance'],
      ['Audit evidence', '#/staff/audit'], ['Reports', '#/staff/reports']
    ]]
  ];
  ...
  function patientTabs(active) {
    const tabs = [
      ['Summary','summary'], ['Demographics','demographics'], ['Encounters','encounters'],
      ['Medications','medications'], ['Labs','labs'], ['Billing','billing']
    ];
  ...
  function shell(content, patientContext = false) {
    const current = route();
    const nav = state.view === 'patient' ? patientNav : staffNav.filter(([, , , roles]) => roles.includes(state.role));
    const identity = state.view === 'patient' ? ['Samira Kamau', 'Patient'] : ['Dr. Elias Okoth', roleLabels[state.role]];
    return `<div class="app-shell">
      <header class="topbar">
        ...
        <div class="user-block"><span class="avatar" aria-hidden="true">${identity[0].split(' ').map(x => x[0]).join('').slice(0,2)}</span><span class="user-meta"><strong>${identity[0]}</strong><span>${identity[1]}</span></span></div>
        ${state.view === 'staff' ? `<label class="sr-only" for="role-preview">Preview role</label><select id="role-preview" class="role-select" data-role>${Object.entries(roleLabels).map(([key,label]) => `<option value="${key}"${key === state.role ? ' selected' : ''}>${label}</option>`).join('')}</select>` : ''}
  ...
  function statesPage() {
    return shell(pageHeader('Shared patterns','System and data states','Reusable references for privacy-safe loading, empty, stale, validation, conflict and service interruption states.') +
  ...
  function patientOverview() {
    const content = pageHeader('Patient portal','Good morning, Samira','Your next steps and recent care activity.',button('Request appointment','primary','appointment-form')) +
      alertBox('Medication pickup ready','Your prescription is ready at Westlands Pharmacy until 13 September.','info') +
  ...
  function patientAppointments() {
    const rows = [
      ['<span class="row-title">Cardiology review</span><span class="row-subtitle">Dr. Wanjiku Muriithi</span>','12 Sep 2026<br><span class="muted">09:30 EAT</span>','Westlands · Clinic 3',status('Confirmed','success'),button('Details','small','notice')],
      ['<span class="row-title">Annual wellness visit</span><span class="row-subtitle">Preferred morning</span>','Requested 08 Sep','Any outpatient clinic',status('Pending','warning'),button('Cancel request','small danger','cancel-appointment')],
      ['<span class="row-title">Telemedicine follow-up</span><span class="row-subtitle">Dr. A. Mwangi</span>','21 Aug 2026<br><span class="muted">14:00 EAT</span>','Virtual consultation',status('Completed','neutral'),button('Summary','small','notice')]
    ];
    return shell(pageHeader('Patient portal','Appointments','Times are shown in East Africa Time (EAT, UTC+3).',button('Request appointment','primary','appointment-form')) +
  ...
  function patientRecords() {
    const rows = [
      ['Clinical summary','06 Sep 2026',status('In review','warning'),'—',button('View request','small','notice')],
      ['Laboratory results','16 Aug 2026',status('Ready','success'),'Available until 16 Oct',button('Download .txt','small primary','download-record')],
      ['Visit summary','02 Jul 2026',status('Downloaded','neutral'),'Expired',button('Request again','small','record-form')]
    ];
    return shell(pageHeader('Patient portal','Health records','Request and securely download copies of your health information.',button('Request records','primary','record-form')) +
      alertBox('Downloads are sensitive','Use a trusted device. Downloaded files may contain private health information and are not saved by this portal.','warning') +
  ...
  function patientBilling() {
    const rows = [
      ['INV-2026-0812','Cardiology consultation','28 Aug 2026','KES 6,850','KES 2,000','<strong>KES 4,850</strong>',status('Part paid','warning')],
      ['INV-2026-0631','Laboratory services','16 Aug 2026','KES 3,200','KES 3,200','KES 0',status('Paid','success')]
    ];
    return shell(pageHeader('Patient portal','Bills and payments','Review balances and submit a secure payment intent.',button('Make a payment','primary','payment-form')) +
  ...
  function patientAccess() {
    return shell(pageHeader('Patient portal','Profile and proxy access','Review contact information and people who can act on your behalf.',button('Add proxy access','primary','proxy-form')) +
  ...
  function workQueue() {
    const rows = [
      [status('Overdue · urgent','danger'),'<span class="row-title">Review critical potassium result</span><span class="row-subtitle">Amara Njeri · MRN-1047-62</span>','Laboratory','08:15 EAT','Unassigned',`<div class="row-actions">${button('Claim','small primary','claim-task')}${button('Open','small','patient-summary')}</div>`],
      [status('Due soon','warning'),'<span class="row-title">Medication reconciliation</span><span class="row-subtitle">Daniel Otieno · MRN-1633-20</span>','Clinical','10:30 EAT','Dr. E. Okoth',button('Open','small','notice')],
      [status('On track','success'),'<span class="row-title">Complete discharge summary</span><span class="row-subtitle">Linet Wambui · MRN-1022-91</span>','Clinical','14:00 EAT','Dr. E. Okoth',button('Open','small','notice')],
      [status('New','info'),'<span class="row-title">Confirm follow-up booking</span><span class="row-subtitle">Kofi Mensah · MRN-1840-05</span>','Operations','Tomorrow','Reception queue',button('Open','small','notice')]
    ];
    return shell(pageHeader('Staff operations','Work queue','Urgent and overdue work appears first. Queue times use EAT.',button('Create task','primary','task-form')) +
      alertBox('1 overdue safety task','Review the critical result before beginning routine queue work.','danger') +
  ...
  function patientsPage() {
    const rows = [
      ['<span class="row-title">Amara Njeri</span><span class="row-subtitle">MRN-1047-62</span>','14 Feb 1988','+254 7•• ••• 901',status('Active','success'),button('Open record','small primary','patient-summary')],
      ['<span class="row-title">Daniel Otieno</span><span class="row-subtitle">MRN-1633-20</span>','09 Jun 1972','+254 7•• ••• 744',status('Active','success'),button('Open record','small','patient-summary')],
      ['<span class="row-title">Linet Wambui</span><span class="row-subtitle">MRN-1022-91</span>','21 Nov 2001','+254 7•• ••• 302',status('Inactive','neutral'),button('Open record','small','patient-summary')]
    ];
    return shell(pageHeader('Staff operations','Patient registry','Search before registering a new patient to reduce duplicate records.',button('Register patient','primary','register-patient')) +
  ...
  function insurancePage() {
    const rows = [
      ['CLM-02418','INV-2026-0812','AfyaCare Gold','KES 12,600','KES 8,000',status('Adjudicated','info'),'08 Sep 2026',button('Open','small','claim-detail')],
      ['CLM-02344','INV-2026-0631','AfyaCare Gold','KES 3,200','KES 3,200',status('Paid','success'),'18 Aug 2026',button('Open','small','notice')],
      ['CLM-02291','INV-2026-0528','AfyaCare Gold','KES 7,450','KES 0',status('Denied','danger'),'02 Aug 2026',button('Review denial','small','notice')]
    ];
    return shell(pageHeader('Revenue cycle','Insurance','Coverage, claims, adjudication and remittance reconciliation.',button('New claim','primary','claim-form') + button('Verify coverage','','coverage-form')) +
  ...
  function pharmacyPage() {
    const rows = [
      [status('Ready for review','warning'),'<span class="row-title">RX-74018 · Amlodipine 5 mg</span><span class="row-subtitle">Daniel Otieno · MRN-1633-20</span>','30 tablets','Dr. E. Okoth','Available',button('Safety review','small primary','safety-review')],
      [status('Safety blocked','danger'),'<span class="row-title">RX-74011 · Amoxicillin 500 mg</span><span class="row-subtitle">Amara Njeri · MRN-1047-62</span>','21 capsules','Dr. E. Okoth','Available',button('View conflict','small danger','safety-conflict')],
      [status('Approved','success'),'<span class="row-title">RX-73998 · Atorvastatin 20 mg</span><span class="row-subtitle">Linet Wambui · MRN-1022-91</span>','30 tablets','Dr. M. Abdalla','Available',button('Dispense','small primary','dispense')]
    ];
    return shell(pageHeader('Pharmacy','Prescription worklist','Dispensing requires safety approval and sufficient unexpired stock.',button('Add prescription','primary','pharmacy-rx')) +
      alertBox('1 prescription blocked','A confirmed allergy conflict must be resolved by the prescribing clinician.','danger') +
  ...
  function inventoryPage() {
    const meds = [
      ['Amlodipine 5 mg','MED-AML-005','1,420','420','31 Dec 2027',status('In stock','success'),button('Movements','small','notice')],
      ['Atorvastatin 20 mg','MED-ATO-020','310','350','30 Nov 2027',status('Below reorder','warning'),button('Receive batch','small primary','batch-form')],
      ['Amoxicillin 500 mg','MED-AMX-500','84','250','10 Sep 2026',status('Expiring soon','danger'),button('Review batches','small','notice')]
    ];
    return shell(pageHeader('Pharmacy','Inventory','Medication and supply movements are recorded against reference IDs.',button('Receive stock','primary','batch-form') + button('Add item','','inventory-item')) +
  ...
  function notificationsPage() {
    const rows = [
      ['NTF-9218','Appointment reminder','Patient · P-••1047','SMS',status('Delivered','success'),'09 Sep · 08:45',button('View','small','notice')],
      ['NTF-9217','Bill alert','Patient · P-••1633','Email',status('Queued','info'),'09 Sep · 08:42',button('View','small','notice')],
      ['NTF-9194','Appointment reminder','Patient · P-••1022','SMS',status('Failed','danger'),'08 Sep · 17:20',button('Retry review','small','notice')]
    ];
    return shell(pageHeader('Operations','Notifications','Track queued and delivered operational messages without exposing message content.',button('New notification','primary','notification-form')) +
  ...
  function compliancePage() {
    const rows = [
      [status('Overdue','danger'),'CASE-2026-041','Potential inappropriate record access','Privacy incident',status('Investigating','warning'),'07 Sep 2026','M. Kirui',button('Open','small primary','case-detail')],
      [status('On hold','warning'),'CASE-2026-037','Records retention request','Data request',status('Remediating','info'),'12 Sep 2026','M. Kirui',button('Open','small','case-detail')],
      [status('On track','success'),'CASE-2026-043','Consent scope review','Privacy review',status('Open','info'),'18 Sep 2026','Unassigned',button('Open','small','case-detail')]
    ];
    return shell(pageHeader('Privacy operations','Compliance cases','Manage evidence, legal holds and valid workflow transitions.',button('Create case','primary','case-form')) +
      alertBox('2 emergency-access reviews pending','The oldest grant expires from the review queue in 37 minutes. Reviewers must be independent.','warning') +
  ...
  function auditPage() {
    const rows = [
      ['09 Sep 2026<br><span class="muted">08:58:14 EAT</span>','user-••27','READ','Patient record','pat-••1047','Clinical care',status('Verified','success'),'REQ-UAT-81BA'],
      ['09 Sep 2026<br><span class="muted">08:45:02 EAT</span>','system-notify','CREATE','Notification','ntf-••9218','Appointment reminder',status('Verified','success'),'REQ-UAT-6A10'],
      ['09 Sep 2026<br><span class="muted">08:15:33 EAT</span>','user-••91','BREAK_GLASS','FHIR record','pat-••1633','Emergency treatment',status('Review pending','warning'),'REQ-UAT-3CD2']
    ];
    return shell(pageHeader('Governance','Audit evidence','Append-only access evidence. Filters use opaque identifiers only.',button('Export filtered evidence','','export-audit')) +
      alertBox('Audit events are read-only','Corrections are appended as new evidence; existing events cannot be changed or removed.','info') +
  ...
  function reportsPage() {
    const trends = `<div class="bar-list">${[['Hypertension follow-up completion',84],['Diabetes HbA1c monitoring',71],['Medication reconciliation',92],['Post-discharge contact',63]].map(([label,pct]) => `<div><div class="bar-head"><strong>${label}</strong><span>${pct}%</span></div><div class="progress-track"><div class="progress-fill" style="width:${pct}%"></div></div></div>`).join('')}</div>`;
    const rows = [
      ['RPT-2026-Q3-01','Quarterly service activity','Regulatory','01 Jul – 30 Sep',status('Draft','warning'),'09 Sep 2026',button('Open','small','notice')],
      ['RPT-2026-08-12','Chronic disease outcomes','Clinical quality','01–31 Aug',status('Ready','success'),'05 Sep 2026',button('Download','small','notice')],
      ['RPT-2026-08-02','Access governance','Compliance','01–31 Aug',status('Submitted','info'),'03 Sep 2026',button('View receipt','small','notice')]
    ];
    return shell(pageHeader('Analytics','Reports','Aggregate operational and population-health information only.',button('Create report','primary','report-form')) +
  ...
  function renderRoute() {
    ...
    const routes = {
      '/login': loginPage, '/forbidden': forbiddenPage, '/states': statesPage,
      '/patient/overview': patientOverview, '/patient/appointments': patientAppointments,
      '/patient/records': patientRecords, '/patient/billing': patientBilling, '/patient/access': patientAccess,
      '/staff/work-queue': workQueue, '/staff/patients': patientsPage,
      ...
      '/staff/schedule': schedulePage, '/staff/insurance': insurancePage,
      '/staff/pharmacy': pharmacyPage, '/staff/inventory': inventoryPage,
      '/staff/notifications': notificationsPage, '/staff/compliance': compliancePage,
      '/staff/audit': auditPage, '/staff/reports': reportsPage
    };
  ...
  function bindEvents() {
    document.querySelectorAll('[data-route]').forEach(el => el.addEventListener('click', () => {
      state.navOpen = false; state.indexOpen = false; location.hash = el.dataset.route;
    }));
    document.querySelectorAll('[data-view]').forEach(el => el.addEventListener('click', () => {
      state.view = el.dataset.view;
      location.hash = state.view === 'patient' ? '#/patient/overview' : '#/staff/work-queue';
    }));
    document.querySelector('[data-role]')?.addEventListener('change', e => { state.role = e.target.value; renderRoute(); });
    document.querySelectorAll('[data-action]').forEach(el => el.addEventListener('click', event => handleAction(event.currentTarget.dataset.action, event.currentTarget)));
    document.querySelectorAll('form[data-submit]').forEach(form => form.addEventListener('submit', e => { e.preventDefault(); state.view = form.querySelector('select')?.value === 'Patient portal' ? 'patient' : 'staff'; location.hash = state.view === 'patient' ? '#/patient/overview' : '#/staff/work-queue'; }));
  }
  ...
  function handleAction(action, source) {
    ...
    const routeActions = {
      home: state.view === 'patient' ? '#/patient/overview' : '#/staff/work-queue',
      logout: '#/login', submit: null,
      'route-appointments': '#/patient/appointments', 'route-access': '#/patient/access',
      'patient-summary': '#/staff/patients/pat-1047/summary', 'route-medications': '#/staff/patients/pat-1047/medications'
    };
    if (routeActions[action]) { location.hash = routeActions[action]; return; }
    if (action === 'toggle-nav') { state.navOpen = !state.navOpen; renderRoute(); return; }
    if (action === 'open-index') { state.indexOpen = !state.indexOpen; renderRoute(); return; }
    if (action === 'change-patient') { showDialog('Change patient context','Opening another record clears cached information for Amara Njeri before the next patient is shown.',`${field('Find patient',input('search','','placeholder="Name or MRN" autofocus'))}${alertBox('Context safety','Confirm identity and date of birth before opening a different record.','warning')}`,'Open selected patient'); return; }
    const dialogs = {
      'appointment-form': ['Request an appointment','Tell us the care you need. A request is not confirmed until scheduling resolves it.', appointmentForm(),'Submit request'],
      'cancel-appointment': ['Cancel appointment request','Cancel the unresolved annual wellness visit request submitted 08 September 2026?',`${field('Reason <span class="required">*</span>',`<textarea required>Plans changed</textarea>`)}${alertBox('This cannot be undone','The request will leave the scheduling queue. You may submit a new request later.','warning')}`,'Cancel request',true],
      'record-form': ['Request health records','Choose only the information and date range you need.',recordForm(),'Submit request'],
      'download-record': ['Download health records','Laboratory results for the period 01–16 August 2026 are ready.',alertBox('Private download','The text file may contain sensitive health information. Save it only on a trusted device.','warning'),'Download records.txt'],
      'payment-form': ['Make a payment','Payment is applied to invoice INV-2026-0812 for Samira Kamau.',paymentForm(),'Submit payment'],
      'proxy-form': ['Add proxy access','Grant one narrow access scope to a trusted person.',proxyForm(),'Create grant'],
      'revoke-proxy': ['Revoke proxy access','Remove Amina Kamau’s Records access for Samira Kamau?',alertBox('Access ends immediately','Amina will no longer be able to view or request records.','warning'),'Revoke access',true],
      'task-form': ['Create staff task','Create work in a permitted queue without adding patient details to the title.',taskForm(),'Create task'],
      'claim-task': ['Claim urgent task','Assign “Review critical potassium result” for Amara Njeri to Dr. Elias Okoth?',alertBox('Clinical responsibility','The task remains overdue until it is completed.','warning'),'Claim task'],
      'register-patient': ['Register patient','Search for duplicates before creating a patient identity.',patientRegistrationForm(),'Review registration'],
      'duplicate-check': ['Potential duplicate review','Compare identity attributes before any merge decision.',duplicateDialog(),'Continue to merge review'],
      'encounter-form': ['Start encounter','Open a clinical encounter for Amara Njeri (MRN-1047-62).',encounterForm(),'Start encounter'],
      'edit-demographics': ['Edit demographics','Changes apply to Amara Njeri after server validation.',demographicsForm(),'Save changes'],
      'lifecycle': ['Change patient lifecycle','Select a valid lifecycle transition for Amara Njeri.',`${field('New status',select(['Inactive','Deceased']))}${field('Reason <span class="required">*</span>',`<textarea required></textarea>`)}${alertBox('Review required','Lifecycle changes affect record access and downstream workflows.','warning')}`,'Confirm status change',true],
      'prescription-form': ['New prescription','Safety checks run before the prescription is created for Amara Njeri.',prescriptionForm(),'Run safety check'],
      'safety-conflict': ['Medication safety conflict','Amoxicillin was blocked for Amara Njeri.',alertBox('Confirmed penicillin allergy','Do not proceed. Select a safe alternative or discontinue the draft prescription.','danger'),'Return to medication list'],
      'ack-alert': ['Acknowledge clinical alert','Confirm that you reviewed the penicillin allergy interaction for Amara Njeri.',`${field('Clinical note <span class="required">*</span>',`<textarea required placeholder="Document your review"></textarea>`)}${alertBox('Acknowledgement is audited','Acknowledging does not remove the allergy or permit unsafe dispensing.','warning')}`,'Acknowledge alert'],
      'lab-review': ['Review critical laboratory result','Potassium 6.2 mmol/L for Amara Njeri, collected 09 Sep 2026 at 08:03 EAT.',`${field('Review outcome <span class="required">*</span>',select(['Contact patient now','Escalate to care team','Result invalid — request repeat']))}${field('Clinical note <span class="required">*</span>',`<textarea required></textarea>`)}`,'Record review'],
      'invoice-form': ['Create invoice','Create a billing item for Amara Njeri in KES.',invoiceForm(),'Create invoice'],
      'posting-form': ['Post payment','Apply a payment to INV-2026-0812. Outstanding balance: KES 4,850.',paymentPostingForm(),'Post payment'],
      'booking-form': ['Book appointment','Confirm patient, clinician, time and duration before booking.',bookingForm(),'Book appointment'],
      'availability-form': ['Add clinician availability','Create an available schedule block in East Africa Time.',availabilityForm(),'Add availability'],
      'claim-form': ['Create insurance claim','Submit one invoice against the verified policy.',claimForm(),'Submit claim'],
      'coverage-form': ['Verify coverage','Request current coverage evidence from the selected payer.',coverageForm(),'Request verification'],
      'claim-detail': ['Claim CLM-02418','Adjudicated for KES 8,000. Remittance has not been reconciled.',claimDetail(),'Record remittance'],
      'safety-review': ['Prescription safety review','Review RX-74018 before approval or dispensing.',safetyReviewForm(),'Approve safety review'],
      'dispense': ['Confirm dispensing','Dispense RX-73998 for Linet Wambui?',dispenseForm(),'Confirm dispense'],
      'pharmacy-rx': ['Add pharmacy prescription','Create a pharmacy work item from a verified clinical prescription.',pharmacyPrescriptionForm(),'Add to worklist'],
      'batch-form': ['Receive stock batch','Record an idempotent stock receipt using the supplier reference.',batchForm(),'Receive batch'],
      'inventory-item': ['Add inventory item','Create a medication catalogue item with zero starting quantity.',inventoryItemForm(),'Create item'],
      'notification-form': ['Create notification','Send an operational notification without including unnecessary clinical detail.',notificationForm(),'Queue notification'],
      'case-form': ['Create compliance case','Evidence references must be identifiers, not patient details or URLs.',caseForm(),'Create case'],
      'case-detail': ['Compliance case CASE-2026-041','Potential inappropriate record access · Current state: Investigating.',caseDetail(),'Move to contained'],
      'export-audit': ['Export audit evidence','Export the currently filtered evidence set.',alertBox('Sensitive evidence','The export contains security metadata. Store and share it only through approved channels.','warning'),'Export CSV'],
      'report-form': ['Create regulatory report','Define the reporting period and intended report type.',reportForm(),'Create draft']
    };
    if (dialogs[action]) { showDialog(...dialogs[action]); return; }
    if (action === 'notice') showToast('Prototype action completed. No data was changed.');
  }
  ...
  function showDialog(title, description, body, confirmText, danger = false) {
    ...
    first?.focus();
    dialogRoot.querySelector('[data-action="close-dialog"]').addEventListener('click', e => { e.preventDefault(); closeDialog(); sourceFocus(); });
    dialogRoot.querySelector('[data-action="confirm-dialog"]').addEventListener('click', e => { e.preventDefault(); e.currentTarget.closest('form').requestSubmit(); });
    dialogRoot.querySelector('[data-dialog-form]').addEventListener('submit', e => { e.preventDefault(); closeDialog(); showToast(`${confirmText} — mockup only. No data was changed.`); sourceFocus(); });
    dialogRoot.querySelector('.dialog-backdrop').addEventListener('click', e => { if (e.target === e.currentTarget) closeDialog(); });
    dialogRoot.addEventListener('keydown', trapDialog);
  }

  let previousFocus;
  function sourceFocus() { previousFocus?.focus(); }
  function closeDialog() { dialogRoot.innerHTML = ''; }
  function trapDialog(e) {
    if (e.key === 'Escape') { closeDialog(); sourceFocus(); }
    if (e.key !== 'Tab') return;
    const els = [...dialogRoot.querySelectorAll('button,input,select,textarea,[href]')].filter(x => !x.disabled);
