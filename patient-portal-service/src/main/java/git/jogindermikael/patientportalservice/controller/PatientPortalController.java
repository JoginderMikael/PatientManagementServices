...
public class PatientPortalController {
    ...
    public ResponseEntity<PortalAppointmentRequest> requestAppointment(@Valid @RequestBody PortalAppointmentCommand request) {
    ...
    public ResponseEntity<RecordAccessRequest> requestRecords(@Valid @RequestBody RecordAccessCommand request) {
        return ResponseEntity.status(HttpStatus.CREATED).body(patientPortalService.requestRecords(request));
    }

    @PostMapping("/payments")
    @Operation(summary = "Submit a patient portal payment")
    public ResponseEntity<PortalPayment> payBill(@Valid @RequestBody PortalPaymentCommand request) {
        return ResponseEntity.status(HttpStatus.CREATED).body(patientPortalService.payBill(request));
    }
}
