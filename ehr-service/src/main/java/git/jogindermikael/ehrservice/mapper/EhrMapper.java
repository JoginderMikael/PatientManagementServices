...
public class EhrMapper {
    ...
    public Diagnosis toDiagnosis(DiagnosisRequest request) {
        return new Diagnosis(UUID.randomUUID(), request.patientId(), request.clinicianId(), request.code(), request.description(), request.diagnosedOn(), Instant.now());
    }

    public Prescription toPrescription(PrescriptionRequest request) {
        return new Prescription(UUID.randomUUID(), request.patientId(), request.clinicianId(), request.medication(), request.dosage(), request.instructions(), "ACTIVE", Instant.now());
    }

    public LabResult toLabResult(LabResultRequest request) {
        return new LabResult(UUID.randomUUID(), request.patientId(), request.testName(), request.resultSummary(), request.source(), request.collectedOn(), Instant.now());
    }

    public LabResult toExternalLabResult(ExternalLabResultRequest request) {
        return new LabResult(UUID.randomUUID(), request.patientId(), request.testName(), request.resultSummary(), "LIS:" + request.externalSystem(), request.collectedOn(), Instant.now());
    }
