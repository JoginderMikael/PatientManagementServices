...
public class FhirPatientController {
    ...
    public FhirPatientController(PatientRepository patients, FhirPrivacyClient privacy) {
    ...
    public Map<String, Object> metadata() {
        return Map.of("resourceType", "CapabilityStatement", "status", "active", "date", "2026-09-08",
                ...
                "implementation",
                Map.of("description", "Patient Management read-only Patient endpoint; consent required"),
                "rest", List.of(Map.of("mode", "server", "resource", List.of(Map.of("type", "Patient",
                        "interaction", List.of(Map.of("code", "read")), "versioning", "no-version", "readHistory",
                        false,
                        "updateCreate", false, "conditionalCreate", false, "conditionalRead", "not-supported",
    ...
    public ResponseEntity<Map<String, Object>> read(@PathVariable String id,
        ...
        try {
            ...
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Invalid Patient identifier");
        }
        ...
        if (patient.getStatus() != PatientStatus.ACTIVE)
            throw new ResponseStatusException(HttpStatus.GONE, "Patient is inactive");
        Map<String, Object> resource = new LinkedHashMap<>();
        resource.put("resourceType", "Patient");
        resource.put("id", id);
        resource.put("meta", Map.of("versionId", Long.toString(patient.getVersion()), "lastUpdated",
        ...
        if (gender != null && Set.of("male", "female", "other", "unknown").contains(gender.toLowerCase(Locale.ROOT)))
            resource.put("gender", gender.toLowerCase(Locale.ROOT));
    ...
    public ResponseEntity<Map<String, Object>> error(ResponseStatusException exception) {
        ...
        return ResponseEntity.status(exception.getStatusCode())
                ...
                .cacheControl(CacheControl.noStore()).body(Map.of("resourceType", "OperationOutcome", "issue",
                        List.of(Map.of("severity", "error", "code", code, "diagnostics",
                                Objects.requireNonNullElse(exception.getReason(), "Request failed")))));
    }
}
