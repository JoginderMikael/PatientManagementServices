...
public class StaffDashboardController {
    ...
    public ResponseEntity<ClinicalRound> createRound(@Valid @RequestBody ClinicalRoundRequest request) {
        return ResponseEntity.status(HttpStatus.CREATED).body(staffDashboardService.createRound(request));
    }

    @PostMapping("/charts")
    @Operation(summary = "Create or update a patient chart summary")
    public ResponseEntity<PatientChartSummary> upsertChart(@Valid @RequestBody PatientChartRequest request) {
        return ResponseEntity.status(HttpStatus.CREATED).body(staffDashboardService.upsertChart(request));
    }
    ...
    public ResponseEntity<StaffTask> createTask(@Valid @RequestBody StaffTaskRequest request) {
        return ResponseEntity.status(HttpStatus.CREATED).body(staffDashboardService.createTask(request));
    }
